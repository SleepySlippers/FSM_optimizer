...
struct fsm
{
    ...
    uint get_number(char *&p, char *q) const
    {
        int c;
        uint r = 0;
        while (p < q)
...
struct Predictor
{
    ...
    void update(uint bit)
    {
        ...
        assert(((ctx >> shift) & 0xFF) == lastbyte);
        assert((((ctx >> shift) >> 8) & 0xFF) == prelastbyte);
        (ctx *= 2) += bit;
        (bits *= 2) += bit;
        ++shift;
    }
...
struct Rangecoder
{
    ...
    uint Process(uint freq, uint bit)
    {
        ...
        // sTOP <= range <= 2^32
        // if bit == 0
        // hotim -> (range * freq * 2 ^ (32 - 15)) / (2 ^ 32) >= sTOP
        // range * freq / (2 ^ 15) >= sTOP
        // (qword(range) * (freq << (32 - SCALElog))) >> 32 >= sTOP
        // qword(range) * (freq << (32 - SCALElog)) >= sTOP << 32
        // (freq << (32 - SCALElog)) >= (sTOP << 32) / qword(range)
        // freq >= (sTOP << 32) / qword(range) >> (32 - SCALElog)
        while (range < sTOP)
        {
            range <<= 8;
            ShiftLow();
...
int main(int argc, char **argv)
{
    ...
    if (argv[1][0] == 'c')
    {
        ...
        rc.Quit();
        printf("%llu -> %llu, compressed: %f\n", qword(f_len), all_chars, double(all_chars) / qword(f_len));
    }
    else
    {
        assert(0);
